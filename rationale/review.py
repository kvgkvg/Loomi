...
def _trusted_summary(connection, version_id: str) -> tuple[dict, dict]:
    ...
    connection.execute(
        ...
        WHERE id = ?
        """,
        (version_id, context["asset_id"]),
    )
    connection.execute(
        """
        UPDATE raw_events
        SET processed = 1, processed_asset_version_id = ?
        WHERE id = (SELECT raw_event_id FROM asset_versions WHERE id = ?)
        """,
...
def review_statement(
    ...
    *,
    ...
    try:
        ...
        if target in {"approved", "edited"} and not connection.execute(
            "SELECT 1 FROM rationale_statements WHERE version_id = ? AND review_status = 'pending' LIMIT 1",
            (row["version_id"],),
        ).fetchone():
